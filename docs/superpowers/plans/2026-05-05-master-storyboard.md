# Master Storyboard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single STORYBOARD.md at the repo root containing the full 7-act, ~18-20 minute conference demo narrative that ties 00-Install, 01-MCP, 03-Skills, and 02-Agents into one cohesive "zero to AI ops agent" walkthrough.

**Architecture:** Single markdown file with prerequisites, 7 sequential acts (each with exact commands, talking points, transitions, and punchlines), and a cleanup section. References pre-written files already in the repo but does not modify them.
...
### Task 1: Create STORYBOARD.md

**Files:**
- Create: `STORYBOARD.md`

- [ ] **Step 1: Create STORYBOARD.md at the repo root**

Create `STORYBOARD.md` with the following content:
...
```bash
wc -l STORYBOARD.md
head -3 STORYBOARD.md
```

Expected: ~300+ lines, first line is `# From Zero to AI Ops Agent — Agentregistry Conference Demo`

- [ ] **Step 3: Commit**

```bash
git add STORYBOARD.md
git commit -m "docs: add master storyboard for end-to-end conference demo"
```

---

### Task 2: Update 00-Install/readme.md

**Files:**
- Modify: `00-Install/readme.md`

The existing file has raw commands with no structure. Update it to be a clean reference that complements the storyboard.

- [ ] **Step 1: Replace 00-Install/readme.md with structured content**

Replace the entire contents of `00-Install/readme.md` with:
...
```bash
git add 00-Install/readme.md
git commit -m "docs: restructure 00-Install readme with sections and storyboard reference"
```
...
- [ ] **Step 1: Verify STORYBOARD.md exists at repo root**
...
- [ ] **Step 2: Verify all 7 acts are present**

```bash
grep "^## Act" STORYBOARD.md
```
...
- [ ] **Step 3: Verify referenced pre-written files exist**

```bash
ls 01-MCP/ops-server/src/tools/get_service_health.py \
   01-MCP/ops-server/src/tools/list_deployments.py \
   01-MCP/ops-server/src/tools/get_logs.py \
...
- [ ] **Step 4: Verify the full repo structure**

```bash
echo "=== Top level ===" && ls *.md && echo "" && echo "=== Folders ===" && ls -d */
```
